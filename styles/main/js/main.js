...
function confirm(url, mess, ajax, functionName, force) {
    if (typeof overrideConfirm === 'function' && force === undefined) {
        return overrideConfirm(url,mess,ajax,functionName);
    }
    iziToast.show({
        theme: 'dark',
        icon: 'icon-person',
...
function notify(m, type, force) {
    if (typeof overrideNotify === 'function' && force === undefined) {
        return overrideNotify(m, type);
    }
    if (type === 'error') {
        iziToast.error({
            message: m,
...
function automationPageInit() {
    if($('#automation-page').length > 0) {
        $.ajax({
            url: buildLink('automation/get/counts', [{key: 'id', value: $('#automation-page').data('id')}]),
            success : function(d) {
                var json = jQuery.parseJSON(d);
                $('.post-count').html(json.posts);
                $('.followers-count').html(json.followers);
                $('.following-count').html(json.following);
            }
        })
    }
}

function removeThisAutoBadge(t) {
    $(t).parent().remove();
    saveAutoBot();
    return false;
}

function addNewAutoBotBadge(tex, container,name) {
    var el = $(tex).emojioneArea();
    var t = el[0].emojioneArea.getText();
    if (t === '') return false;
    $(container).prepend("<span class='badge badge-light m-2 each-autobot-badge'>" +
        "<input type='hidden' name='val[settings]["+name+"][]' value='"+t+"'/>" +
        ""+t+"" +
        "<a href='' onclick='return removeThisAutoBadge(this)'><i class='la la-times'></i></a>" +
        "</span>");
...
function saveAutoBot(t) {
    $('#automation-page').submit();
    if (t !== undefined)  {
        var type = $(t).data('type');
        var checked = $(t).prop('checked');
        if (checked) {
            $('#'+type+'-toggle-button').fadeIn();
        } else {
            $('#'+type+'-toggle-button').fadeOut();
        }
    }
    return false;
}
...
function reloadInit(paginate) {
    ...
    if ($("#filemanager-container").length > 0) {
        try {
            Dropzone.forElement(".uploads-files-container").destroy();
        } catch(e){}
        try {
            $(".uploads-files-container").dropzone({
                url: $(".uploads-files-container").data('upload-url'),
                uploadMultiple: true,
                ...
                init: function() {
                    ...
                    this.on('completemultiple', function(result) {
                        ...
                        var result = jQuery.parseJSON(result[0].xhr.response);
            ...
            if ($(this).scrollTop() === 0) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        contentContainer.prepend(json.content);
    ...
    $('.each-file-pane').each(function() {
        if ($(this).data('loaded') === undefined) {
            if ($(this).data('type') === 'image') {
                $(this).css("background-image", 'url('+$(this).data('file')+')');
            } else {
                $(this).find('.video-content').html('<video src="'+$(this).data('file')+'" playsinline="" muted="" loop=""></video>')
            }
    ...
    if($("#facebookSelectModal").length > 0) {
        $("#facebookSelectModal").modal('show');
    }

    if($("#pinterestSelectModal").length > 0) {
        $("#pinterestSelectModal").modal('show');
    }

    if($("#linkedinSelectModal").length > 0) {
        $("#linkedinSelectModal").modal('show');
    }
    if($("#googleSelectModal").length > 0) {
        $("#googleSelectModal").modal('show');
    }
    ...
    if($("#schedule-calender").length > 0 && $("#schedule-calender").html() === '') {
        ...
        setInterval(function() {
            $('.fc-daygrid-day').each(function() {
                ...
                date = month + '/' + day + '/' + year;
                if ($(this).find('.create-post-link').length < 1) $(this).append("<a title='"+strings.create_new_post+"' data-ajax='true' href='"+baseUrl+"post?date="+date+"' class=' create-post-link mtitle' ><i class=\"las la-plus-circle\"></i></a>")
            ...
            render: {
                option: function(data, escape) {
                    return '<div class="option">' +
                        '<span class="image"><img style="width:30px;height:30px;border-radius:100px" src="' + escape(data.url) + '"/></span>' +
                        '<span class="title" style="color:#000 !important;margin-left:10px;font-size:11px;">' + escape(data.title) + '</span>' +
                        '</div>';
                },
                item: function(data, escape) {
                    return '<div class="item"><img style="width:20px;height:20px;border-radius:100px" src="'+ escape(data.url) +'"/> <span style="color:#000 !important;margin-left:10px;font-size:11px;">'+ escape(data.title) +'</span></div>';
                }
            },
            create: false
        });
    ...
    $( ".uploads-files-container" ).sortable({
        items: '.sort',
        stop: function() {
            adjustFileSorting();
        }
    ...
    $( ".party-list-container" ).sortable({
        items: '.each-party-container',
        stop: function() {
            var form = $('.party-list-container').parent();
            form.submit();
        }
    ...
    $( ".party-posts-list" ).sortable({
        items: '.each-party-post',
        stop: function() {
            var posts = [];
            $('.each-party-post').each(function() {
                posts.push($(this).find('.each-post-input').val());
            })
            pageLoader(true)
            $.ajax({
                url : window.location.href,
                data: {sort: 1, posts: posts},
                success: function() {
                    pageLoaded();
                }
            });
...
function adjustFileSorting() {
    var form = $('.uploads-files-container').parent();
    form.find('.action').val('sort');
    form.submit();
}
...
function finishPinterestApiLogin(c) {
    $('#addPinterestModal').modal('hide');
    $("#pinterestAPISelectModal").find('.modal-body').html(c);
    $("#pinterestAPISelectModal").modal('show')
    return false;
}
...
$(function() {
    ...
    $(document).on('click', '.fc-daygrid-event',function() {
        if ($(this).hasClass('page-link')) return load_page($(this).attr('href'));
        pageLoader(true);
        $.ajax({
            url: $(this).attr('href'),
            success : function(r) {
    ...
    $(document).on('click', '.search-container .dropdown-menu a', function() {
        var title = $(this).data('title');
        var icon = $(this).data('icon');
        var link = $(this).data('link');
        $(".search-container").find('input').attr('placeholder', title);
        $(".search-container").find('.dropdown-toggle').html(icon);
        $(".search-container").find('form').prop('action', link);
        $('body').click();
        return false;
    });
...
function preivewDesign(t, p) {
    ...
    $('#app-container .side-menu .active i, #app-container .side-menu .active span,#horizontal-menu .menu-container .active span,.thin-menu ul a:hover,.thin-menu ul a.active').each(function() {
        this.style.setProperty('color', p, 'important');
    });
    $('.thin-menu ul a.active,.btn-primary').each(function() {
        this.style.setProperty('border-color', p, 'important');
    });

    $('.compose-btn').each(function() {
        var fColor = json['compose-first'];
        var sColor = json['compose-second'];
        this.style.setProperty('background', 'linear-gradient(45deg, '+fColor+' 0%,  '+fColor+' 40%,  '+sColor+' 60%,  '+sColor+' 100%) ', 'important');

    });

    $('#app-container .left-pane').each(function () {
        this.style.setProperty( 'background-color', json['col-menu-bg'], 'important');
    });

    $('.thin-menu ul a').each(function() {
        this.style.setProperty('color', json['col-menu-link-color'], 'important');
    });

    $('#report-icon-color').each(function() {
        this.style.setProperty('color', json['report-icon-color'], 'important');
    });
    $('#rss-icon-color').each(function() {
        this.style.setProperty('color', json['rss-icon-color'], 'important');
    });
    $('#captions-icon-color').each(function() {
        this.style.setProperty('color', json['captions-icon-color'], 'important');
    });
    $('#filemanager-icon-color').each(function() {
        this.style.setProperty('color', json['filemanager-icon-color'], 'important');
    });
    $('#groups-icon-color').each(function() {
        this.style.setProperty('color', json['groups-icon-color'], 'important');
    });
    $('#affiliate-icon-color').each(function() {
        this.style.setProperty('color', json['affiliate-icon-color'], 'important');
    });

    $('#app-container .side-menu .active').each(function () {
        this.style.setProperty( 'background-color', json['nav-active-background-color'], 'important');
    });
    $('.opened-menu .left-pane').each(function () {
        this.style.setProperty( 'background-color', json['exp-menu-bg'], 'important');
    });

    $('.opened-menu .thin-menu ul a').each(function() {
        this.style.setProperty('color', json['exp-menu-link-color'], 'important');
    });

    $('.menu-toggle, .sub-menu-toggle').each(function() {
        this.style.setProperty('color', json['menu-toggle-color'], 'important');
    });
    $('.menu-toggle, .sub-menu-toggle').each(function() {
        this.style.setProperty('background-color', json['menu-toggle-bg-color'], 'important');
    });
    $('.menu-toggle, .sub-menu-toggle').each(function() {
        this.style.setProperty('border-color', json['menu-toggle-border-color'], 'important');
    });
    return false;
}
...
function uploadFinished(image) {
    ...
    if($('.create-folder-item').length > 0 && $('.last-folder-item').length < 1) {
        $('.uploads-files-container').find('.create-folder-item').after(image);
        reloadInit();
        adjustFileSorting();
        return true;
    }

    if ($('.last-folder-item').length > 0) {
        $('.uploads-files-container').find('.last-folder-item').after(image);
        reloadInit();
        adjustFileSorting();
        return true;
    } else {
        $('.uploads-files-container').prepend(image);
        reloadInit();
        adjustFileSorting();
        return true;
    }
...
function deleteSelectFiles() {
    ...
    if (selected > 0) {
        confirm('','confirm-file-delete','function',function() {
...
function confirmFileSort() {
    $('.uploads-files-container').parent().find('.action').val(1);
    return false;
}
...
function singlePreviewFile(t, file , type) {
    if (type === 'image') {
        BigPicture({
            el: t,
            imgSrc: file,
...
function confirmFolderCreate(c) {
    ...
    if ($('.create-folder-item').length > 0) {
        $('.create-folder-item').after(c);
    } else {
...
function startVk(t) {
    var o = $(t);
    window.open(o.data('url'),'','toolbar=0,status=0,width=726,height=536');
    $('.vk-button').removeProp('disabled');
    $('.vk-button').removeAttr('disabled');

    return false;
}

function openVK() {
    $('#vkCodeModal').modal('show');
    return false;
}

function vkCodeSuccess(c) {
    $('#vkCodeModal').find('.the-content').html(c);
    $('#vkCodeModal').find('.code-button').remove();
    $('#vkCodeModal').find('.vk-button').html(strings.add_account)

}

function fetchGBLocations(t) {
    var o = $(t);
    ...
    o.css('opacity', '0.4');
    $.ajax({
        url : buildLink('accounts/google', [{key:'fetch', value: value}]),
        success: function(data) {
            container.html(data);
            o.css('opacity', '1');
            $("#google-btn-submit").removeProp('disabled');
            $("#google-btn-submit").removeAttr('disabled')
        }
    });
    return false;
}
...
function openAccountSelector() {
    ...
    $("#addAccountModal").find('.modal-body').html("<div class='loader-image'><img src='"+loaderImage+"'/></div>");
    var i = $("#addAccountModal").find('.modal-header').find('select').val();
    var rss = ($('.rss-container').length > 0 && $('.posts-rule-container').length < 1) ? 1 : 0;
    $.ajax({
        url: buildLink('post', [{key: 'load', value: i}, {key: 'rss' , value: rss}]),
        success: function(data) {
            $("#addAccountModal").find('.modal-body').html(data);
        }
    })
    return false;
}
...
function loadGroupSelector() {
    var v = $("#groups-select-input").val();
    $("#addAccountModal").find('.modal-body').html("<div class='loader-image'><img src='"+loaderImage+"'/></div>");
    var rss = ($('.rss-container').length > 0) ? 1 : 0;
    $.ajax({
        url: buildLink('groups', [{key: 'load', value: v}, {key: 'rss' , value: rss}]),
        success: function(data) {
            $("#addAccountModal").find('.modal-body').html(data);
        }
    });
...
function selectThisAccount(t, checkbox) {
    var o = $(t);
    ...
    if (o.hasClass('active')) {
        ...
        o.find('input').removeProp('checked');
...
function renderSelectedAccounts() {
    var c = $("#addAccountModal").find('.modal-body');
    var count = 0;
    window.notifyYoutubeError = true;
    c.find('.each').each(function() {
        if ($(this).find('input').prop('checked') == true) {
            ...
            if ($(".each-"+id).length < 1) {
                ...
                $(".middle #horizontal-menu .menu").append("<div class='menu-item mtitle each-"+id+"' data-type='"+type+"' title='"+typeTitle+"  - "+name+"' ><img src='"+image+"'/><input type='hidden' value='"+id+"' name='val[accounts][]'/><a onclick='return removeAccountPost(this)' href=''><i class=\"las la-times\"></i></a></div>")
    ...
    if (count > 0) {
        ...
        notify(strings.select_an_account_to_continue)
    }

    finilizeSelectedAccounts();
}

function renderRssSelectedAccounts() {
    var c = $("#addAccountModal").find('.modal-body');
    var count = 0;
    window.notifyYoutubeError = true;
    c.find('.each').each(function() {
        if ($(this).find('input').prop('checked') == true) {
            ...
            if ($(".each-"+id).length < 1) {
                ...
                var name = $(this).data('name');
                var typeTitle = type.charAt(0).toUpperCase() + type.slice(1);
                $(".middle #horizontal-menu .menu").append("<div class='menu-item mtitle each-"+id+"' data-type='"+type+"' title='"+typeTitle+"  - "+name+"' ><img src='"+image+"'/><input type='hidden' value='"+id+"' name='accounts[]'/><a onclick='return removeAccountRss(this)' href=''><i class=\"las la-times\"></i></a></div>")
    ...
    $('#rss-accounts-form').submit();
}

function removeAccountPost(t) {
    $(t).parent().remove();
    finilizeSelectedAccounts();
    return false;
}

function removeAccountRss(t) {
    $(t).parent().remove();
    $('#rss-accounts-form').submit();
    return false;
}
...
function useCaption(t) {
    ...
    el[0].emojioneArea.setText($(t).find('.value').html());
    closeLoadCaptions();
    return false;
}
...
function switchInstagramMenu(type, t) {
    ...
    $(".watermark-form").hide();
    $(".story-form").hide();
    $('.title-input').removeAttr('required');
    removeAllSelectdFiles()
    switch(type) {
        case 'media':
            fileSelectorMediaType = 2;
        ...
        case 'igtv':
            fileSelectorMediaType = 1;
            fileSelectorType = 0;
            $("#post-type-input").val("igtv");
            $(".instagram-post-form").hide();
            $(".watermark-form").hide();
            $('.media-preview').show();
            $('.title-input').show();
            $('.title-input').attr('required', 'required');
            $('.title-input').prop('required', 'required')
            break;
        case 'livestream':
            fileSelectorMediaType = 1;
            fileSelectorType = 0;
...
function displayGeneralOneMedia(medias, container) {
    ...
    if(media.type === 'image') {
        div.data('image', media.url.replace(baseUrl, ''));
        div.css("background-image", 'url(' + media.url+')')
    } else {
        div.html("<video src='"+media.url+"' playsinline='' muted='' loop=''></video>")
    }
...
function previewInstagram(medias) {
    ...
    if ($("#instagram-post-menu").css('display') !== 'none' ) {
        ...
        if ($("#instagram-post-menu a:nth-child(2)").hasClass('active')) {
            ...
            if ($("#instagram-post-menu a:nth-child(4)").hasClass('its-album')) {
        ...
        } else if($("#instagram-post-menu a:nth-child(5)").hasClass('active')) {
            $('.instagram-normal').show();
            displayInstagramMedia(medias, true)
        } else {
            $('.instagram-normal').show();
...
function loadInboxThread(t, social, pre) {
    pageLoader(true);
    $.ajax({
        url : buildLink('inbox/load/threads'),
        data : {social: social, id: $(t).val()},
        success: function(d) {
            $("#social-threads-"+social).html(d);
            if($("#social-threads-"+social).find('.each-thread').length > 0) {
                var firstThread = $("#social-threads-"+social).find('.each-thread').first();
                if (pre === undefined) openInboxThread(firstThread, firstThread.data('thread'), firstThread.data('social'), firstThread.data('account'))
            }
            pageLoaded();
            reloadInit();
        }
...
window.chatopen = null;
window.chatThread = null;
window.chatSocial = null;
window.chatAccount = null;
function openInboxThread(t, threadId, social, id) {
    $(".social-"+social+"-thread-content a").removeClass('active');
    $(t).addClass('active');
    window.chatopen = t;
    window.chatThread = threadId;
    window.chatSocial = social;
    window.chatAccount = id;
    pageLoader();
    $.ajax({
        url : buildLink('inbox/load/chat'),
        data:{social:social,thread:threadId,id:id},
        success:function(d) {
            $('.chat-container').html(d);
            pageLoaded();
            reloadInit();
            setTimeout(function() {
                $("#chat-list").scrollTop($("#chat-list").prop("scrollHeight"));
            }, 300);
            autoPaginateScroll($("#chat-list"), true);
...
function autoPaginateScroll(t, again) {
    var mainContainer = $(t);
    ...
    $.ajax({
        url: $(t).data('url'),
        data:{offset: $(t).data('offset'), paginate: true},
        type: 'POST',
        success: function(result) {
            var json = jQuery.parseJSON(result);
            mainContainer.data('offset', json.offset);
...
function submit_chat_form() {
    $('#chat-form').submit();
    return false;
}

function render_send_chat(c) {
    $('.chat-list-content').append(c);
    $("#chat-list").scrollTop($("#chat-list").prop("scrollHeight"));
    $("#chat-form textarea").data("emojioneArea").setText("");
    $("#chat-form input[type=file]").val('');
    return false;
}

function reload_chat_pane() {
    $('.social-tabs').each(function() {
        if ($(this).css('display') !== 'none') {
            var account = $(this).find('.input-select-custom').val();
            if (account !== '') {
                var social = $(this).data('social');
                loadInboxThread($(this).find('.input-select-custom'), social, false);
                if (window.chatopen !== null) {
                    openInboxThread(window.chatopen, window.chatThread, window.chatSocial, window.chatAccount);
                }
            }
        }
    })
