...
class AdminModel extends Model {
    ...
    public function saveDesign($val, $id = null) {
        $ex = array(
            'title' => '',
            'primary' => '',
            'data' => array()
        ...
        $data = perfectSerialize($data);
        if ($id) {
            $this->db->query("UPDATE designs SET title=?,primary_color=?,data=? WHERE id=?", $title, $primary, $data, $id);
        } else {
            $this->db->query("INSERT INTO designs (title,primary_color,data)VALUES(?,?,?)", $title, $primary, $data);
        }
        return true;
    }
    ...
    public function findDesign($id) {
        $query = $this->db->query("SELECT * FROM designs WHERE id=?", $id);
        return $query->fetch(PDO::FETCH_ASSOC);
    }

    public function setDefaultDesign($id) {
        $this->db->query("UPDATE designs SET is_default=?", 0);
        return $this->db->query("UPDATE designs SET is_default=? WHERE id=?", 1, $id);
    }

    public function deleteDesign($id) {
        if ($id == 1) return false;
        return $this->db->query("DELETE FROM designs WHERE id=?", $id);
    }

    public function getPageDesign() {
        $user = model('user')->authUser;
        $id = $user['default_color'];
        return $this->findDesign($id);
    }
    ...
    public function monthlyStatistics() {
        ...
        foreach($months  as $month) {
            $startDate = strtotime("first day of $month $year");
