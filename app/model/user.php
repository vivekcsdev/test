...
class UserModel extends Model {
    ...
    public function addUser($val, $isAdmin = false, $noActivate = false) {
        ...
        Hook::getInstance()->fire('user.signup.finished', null, array($userid, $val));
        ...
        if (!$noActivate) {
            ...
            if (!$isAdmin) Hook::getInstance()->fire('user.signup.success', null, array($userid, $val));
    ...
    public function getAllSocials() {
        ...
        $socials = Hook::getInstance()->fire('user.all.socials', $socials);
        return $socials;
    }
    public function getAvailableSocials() {
        $socials = array();
        if ($this->canUseSocial('facebook')) $socials[] = 'facebook';
        if ($this->canUseSocial('twitter')) $socials[] = 'twitter';
        if ($this->canUseSocial('instagram')) $socials[] = 'instagram';
        if ($this->canUseSocial('google')) $socials[] = 'google';
        if ($this->canUseSocial('pinterest')) $socials[] = 'pinterest';
        // if ($this->canUseSocial('vk')) $socials[] = 'vk';
        if ($this->canUseSocial('linkedin')) $socials[] = 'linkedin';
        if ($this->canUseSocial('youtube')) $socials[] = 'youtube';
        if ($this->canUseSocial('whatsapp')) $socials[] = 'whatsapp';
        if ($this->canUseSocial('tiktok')) $socials[] = 'tiktok';
        if ($this->canUseSocial('snapchat')) $socials[] = 'snapchat';
        if ($this->canUseSocial('rss')) $socials[] = 'rss';
        // if ($this->canUseSocial('tumblr')) $socials[] = 'tumblr';
        // if ($this->canUseSocial('vimeo')) $socials[] = 'vimeo';
        // if ($this->canUseSocial('reddit')) $socials[] = 'reddit';
        // if ($this->canUseSocial('telegram')) $socials[] = 'telegram';
        // if ($this->canUseSocial('dailymotion')) $socials[] = 'dailymotion';

        $socials = Hook::getInstance()->fire('user.available.socials', $socials);
        return $socials;
    }

    public function canUseSocial($social) {
        if (!config("enable-$social-integration", true)) return false;
        if (!moduleExists('saas')) return true;
        $permissions = $this->getPermissions();
        if (empty($permissions)) return true;
        if (isset($permissions[$social])) return $permissions[$social];
        return false;
    }
    ...
    public function sendInviteCode($id) {
        ...
        return Email::getInstance()->setAddress($email, $name)
            ...
            ->setMessage(l('invite-member-message',array( 'name' => $name, 'link' => $link, 'owner' => $this->authOwner['full_name'])))
            ->send();
    }
    ...
    public function teamCanUse($p) {
        ...
        return (isset($permission[$p])) ? $permission[$p] : true;
    }
