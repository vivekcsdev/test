...
class HomeController extends Controller {
    ...
    public function index() {
        ...
        $this->setWrapLayout('home/layout');
        return $this->render($this->view('home/index'), true);
    }
    ...
    public function signup() {
        if (!config('user-signup', true)) return $this->request->redirect(url(''));
        if ($this->model('user')->isLoggedIn()) return $this->request->redirect(url('post'));
        $this->setWrapLayout('auth/layout');
        if ($val = $this->request->input('val')) {
            ...
            if ($validator->passes()) {
                ...
                if (config('email-verification', false)) {
                    ...
                    return json_encode(array(
                        'message' => l('registeration-successful'),
                        'type' => 'normal-url',
                        'value' =>getHomeUrl()
                    ));
                }
    ...
    public function activate() {
        ...
        if($time > time() - 172800) {
            if ($user = $this->model('user')->findByActivationCode($code)) {
                ...
                return $this->request->redirect(getHomeUrl());
            }
        }
    ...
    public function facebookAuth() {
        ...
        if ($code = $this->request->input('code')) {
            ...
            $email = (isset($user->email)) ? $user->email : $user->id.'@facebook.com';
            $val = array(
                'full_name' => $user->name,
                'email' => $email,
                'password' => time().$email,
            ...
            if ($user = $this->model('user')->getUser($email)) {

                $this->model('user')->loginWithObject($user);

                return $this->request->redirect(getHomeUrl());
            } else {
                $userid = $this->model('user')->addUser($val , false, true);
                if (config('email-verification', false)) {
                    Database::getInstance()->query("UPDATE users SET status=? WHERE id=?", 1, $userid);
                    $this->model('user')->loginUser($val['email'], $val['password'], true);
                    return $this->request->redirect(getHomeUrl());
                } else {
                    $this->model('user')->loginUser($val['email'], $val['password'], true);
    ...
    public function googleAuth() {
        if ($code = $this->request->input('code')) {
            ...
            $googleAPI->setClientSecret(config('google-client-id'), config('google-client-secret'));
            ...
            if ((!isset($user['email']) or !$user['email'])) return $this->request->redirect(url('login'));
            $email = (isset($user['email']) and $user['email']) ? $user['email'] :null;
    ...
    public function openMenu() {
        session_put('open_menu', 1);
    }

    public function closeMenu() {
        session_put('open_menu', 0);
    }

    public function goBackAdmin() {
        unset($_SESSION['shadow_userid']);
        return json_encode(array(
            'type' => 'normal-url',
            'value' => url('admin/users'),
            'message' => l('you-viewing-your-account')
        ));
    }
}