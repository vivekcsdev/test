...
class Translator {
    ...
    public function __construct()
    {
        ...
        $query = Database::getInstance()->query("SELECT * FROM languages WHERE is_default=?", 1);
        $result = $query->fetch(PDO::FETCH_ASSOC);
        if ($result) $this->lang = $result['id'];
    ...
    public function loadDatabaseStrings() {
        ...
        $query  = Database::getInstance()->query("SELECT lang_key,translated FROM translations WHERE lang=? ", $this->lang);
        while($string = $query->fetch(PDO::FETCH_ASSOC)) {
            $this->dbLoaded[$string['lang_key']] = $string['translated'];
        }
